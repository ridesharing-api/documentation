...
def table_body(propspace, schema, typespace):
    ...
    for prop_name, prop in schema["properties"].items():
        ...
        elif prop_name in RidesharingApi.default_properties:
            description = ""
        else:
...
def localize_schema(language, translations_file, schema_file):
    ...
    with open(translations_file) as f:
        translations = yaml.load(f, Loader=yaml.FullLoader)[language]
...
def schema_to_markdown(schema_folder, examples_folder, output_file, language, language_file):
    ...
    assert (len(RidesharingApi.objects) == len(os.listdir(schema_folder)) - 1)

    generated_schema = ""

    for obj in RidesharingApi.objects:
