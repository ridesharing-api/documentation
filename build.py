#!/usr/bin/env python3
# This program is part of MetaMitfahrPort and may be used to build the specification
# It's heavily inspired by OParl and its build system: https://github.com/OParl/spec
...
def get_filename_base(language, version):
    return 'ridesharing-api-{}-{}'.format(version, language)
...
def create_symlinks(filename_base):
    ...
    for specfile in os.listdir('build/{}'.format(filename_base)):
        ...
        new = 'build/latest/ridesharing-api{}'.format(os.path.splitext(specfile)[-1])
